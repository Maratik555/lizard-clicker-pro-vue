...
export async function getOrCreaateUser() {
  const potentialUser = await supabase
    .from('users').select().eq('telegram', MY_ID);
...
export async function registerRef(userName, refId) {
  const {data} = await supabase
    .from('users').select().eq('telegram', +refId);
...
export async function completeTask(user, task) {
  ...
  await supabase
    .from('users').update({tasks: {...user.tasks, [task.id]: true}, score: newScore}).eq('telegram', MY_ID)
}